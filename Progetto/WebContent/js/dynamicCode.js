function dynamicIndex(url) {
	$.ajax({
		...
		for (const prodotto of response) {
			...
			contenutoHtml += "<a href=\"ProductServlet?codice=" + prodotto.codice + "\"><img src=\"" + prodotto.img + "\"> </a>";
			contenutoHtml += "<div class=\"info\">";
			contenutoHtml += "<h4>" + prodotto.nome + "</h4>";
			contenutoHtml += "<p>&#8364 " + prodotto.prezzo.toFixed(2) + "</p>";
			contenutoHtml += "<a onclick=\"addCart(" + prodotto.quantita + ", '" + prodotto.codice + "')\"> Carrello</a>";
	...
	$.ajax({
		...
		if (response.url === undefined) {
			for (const p of response) {
				contenutoHtml += "<tr>";
				contenutoHtml += "<td> <button data-codice='" + p.codice + "'onclick=eliminaRiga(this)><img src=\"./icons/trash.ico\" class=trash></button>";
...
function dynamicCatalog(url) {
	$.ajax({
		...
		for (const p of response) {
			contenutoHtml += "<div class=\"scheda\" data-categoria=\"" + p.categoria + "\" data-marca=\"" + p.marca + "\">";
			contenutoHtml += "<a href=\"ProductServlet?codice=" + p.codice + "\"><img src=\"" + p.img + "\" class=\"trash\"></a>";
			contenutoHtml += "<div class=\"info\">" + "\n";
			contenutoHtml += "<h4 class=\"pname\">" + p.nome + "</h4>" + "\n";
			contenutoHtml += "<p> &#8364 " + p.prezzo.toFixed(2) + "</p>" + "\n";
			contenutoHtml +=  "<a onclick=\"addCart(" + p.quantita + ", '" + p.codice + "')\"> Carrello</a>";
			contenutoHtml += "</div>";
			contenutoHtml += "</div>";
		}
		$("#schedeProdotto").append(contenutoHtml);
...
function dynamicGeneri(url) {
	$.ajax({
		...
		let filtriMarca = "<tr> <td> <h4> Genere </h4> </td> </tr>";
		for (const marca of response) {
			filtriMarca += "<tr> <td>";
			filtriMarca += "<input type=\"checkbox\" class=\"mar\" name=\"marca\" value=\"" + marca + "\" onchange=\"searchAndFilter()\">";
			filtriMarca += "<label class=\"firstset\">" + marca + "</label>";
			filtriMarca += "</td> </tr>";
		}
		$("#marche").append(filtriMarca);
	});
}
